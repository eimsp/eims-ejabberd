...
extended_fields() ->
	[{withtext, <<"">>},
		#xdata_field{var = <<"with_nick">>, label = <<"Search by nick">>, type = 'text-single'},
		#xdata_field{var = <<"with_xml">>, label = <<"Search by xml">>, type = 'text-single'}].
...
select(LServer, JidRequestor, #jid{luser = LUser} = JidArchive,
	...
	Extra = case {proplists:get_value(with, MAMQuery), Flags} of
		        {#jid{luser = Name, lserver = Service}, {_, true}} ->
			        case catch mod_muc_admin:get_subscribers(Name, Service) of
				        {error, _} -> none;
				        Subs -> [(jid:decode(Sub))#jid.luser || Sub <- Subs]
			        end;
				_ -> none
	        end,

	{Query, CountQuery} = make_sql_query(User, LServer, MAMQuery, RSM, Extra),
...
select_with_mucsub(LServer, JidRequestor, #jid{luser = LUser} = JidArchive,
		   MAMQuery, RSM, Flags) ->
	Extra =
		case gen_mod:db_mod(LServer, mod_muc) of
			mod_muc_sql ->
				subscribers_table;
			_ ->
				case {proplists:get_value(with, MAMQuery), Flags} of
					{#jid{luser = Name, lserver = Service}, {_, true}} ->
						case catch mod_muc_admin:get_subscribers(Name, Service) of
							{error, _} -> [];
							Subs -> [(jid:decode(Sub))#jid.luser || Sub <- Subs]
					...
					_ ->
						case mod_muc_admin:find_hosts(LServer) of
							[First | _] ->
								case mod_muc:get_subscribed_rooms(First, JidRequestor) of
									{ok, L} -> [jid:encode(Jid) || {Jid, _, _} <- L];
									{error, _} -> []
								end;
							_ ->
								[]
...
do_select_query(LServer, JidRequestor, #jid{luser = LUser} = JidArchive, RSM,
	...
	{Flags2, NotAnon} = case Flags of {F, _} -> {F, true}; _ -> {Flags, false} end,
    QRes = case Flags2 of
		   all ->
...
%%		IsModerator = case proplists:get_value(with_subs, )
	    {lists:flatmap(
		fun([TS, XML, PeerBin, Kind, Nick]) ->
		   ...
		   ([User, TS, XML, PeerBin, Kind, Nick]) when User == LUser; NotAnon ->
		       case make_archive_el(JidArchiveS, TS, XML, PeerBin, Kind, Nick,
					    MsgType, JidRequestor, JidArchive) of
			   {ok, El} ->
			       [{TS, binary_to_integer(TS), El}];
     ...
     {archive_msg,
      fun([Host | HostTail], #archive_msg{us ={LUser, LServer},
          ...
          when (LServer == Host) or ([LServer] == HostTail)  ->
				...
				OriginId = eims:get_origin_id(Pkt,_ID),
                case SqlType of
	                mssql -> [?SQL_INSERT(
	                    "archive",
	                    ["username=%(SUser)s",
...
make_sql_query(User, LServer, MAMQuery, RSM, ExtraUsernames) ->
    ...
    WithNick = proplists:get_value(with_nick, MAMQuery),
    WithXml = proplists:get_value(with_xml, MAMQuery),
	WithOriginId = proplists:get_value(with_origin_id, MAMQuery),
	WithRetractId = proplists:get_value(with_retract_id, MAMQuery),
...
%%	WithTextClause =
%%		case WithText of
%%			_ when is_binary(WithText), WithText /= <<>> ->
...
%%		     end,
	WithTextClause =
		case WithText of
			 _ when is_binary(WithText), WithText /= <<>> ->
				 [<<" and to_tsvector(txt) @@  to_tsquery(">>,
					 ToString(WithText), <<")">>];
			not_empty ->
				[<<" and txt!=">>, ToString(<<>>)];
			_ -> []
	...
	WithNickClause =
		if is_binary(WithNick), WithOriginId /= <<>> ->
			     [<<" and nick=">>,
			      ToString(WithNick)];
			true -> []
		end,
	WithXmlClause =
		if is_binary(WithXml), WithOriginId /= <<>> ->
			     [<<" and xml like ">>,
			      ToString(WithXml)];
			true -> []
		end,
    WithOriginIdClause = if is_binary(WithOriginId), WithOriginId /= <<>> ->
			     [<<" and origin_id=">>,
			      ToString(WithOriginId)];
			true ->
			     []
		     end,
    WithRetractIdClause = if is_binary(WithRetractId), WithRetractId /= <<>> ->
			     [<<" and retract_id=">>,
			      ToString(WithRetractId)];
			true ->
			     []
		     end,
    ...
    Query = [<<"SELECT ">>, TopClause, UserSel,
	     ...
	     WithClause, WithTextClause, WithOriginIdClause, WithRetractIdClause, WithNickClause, WithXmlClause,
    ...
    {QueryPage,
     [<<"SELECT COUNT(*) FROM archive WHERE ">>, UserWhere,
      HostMatch, WithClause, WithTextClause, WithOriginIdClause, WithRetractIdClause, WithNickClause, WithXmlClause,
