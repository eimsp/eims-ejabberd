...
process_iq(LServer, #iq{id = Id, from = #jid{luser = LUser} = From, lang = Lang,
				...
				case Id of
					<<"moderator_", _/binary>> ->
						case proplists:get_value(with, Query) of
							#jid{luser = Name, lserver = Service} ->
								case catch mod_muc_admin:get_room_occupants(Name, Service) of
									{error, room_not_found} ->
										xmpp:make_error(IQ,
											xmpp:err_forbidden(<<"Moderator request is not allowed. The groupchat ",
												(jid:to_string({Name, Service, <<>>}))/binary, " not found">>, Lang));
									Occupants ->
										case lists:keyfind(jid:encode(From), 1, Occupants) of
											{_, _, "moderator"} ->
												NewRSM = limit_max(RSM, NS),
												select_and_send(LServer, Query, NewRSM, FlipPage, IQ, MsgType);
											_ ->
												xmpp:make_error(IQ, xmpp:err_forbidden(?T("You are not moderator"), Lang))
										end
								end;
							undefined ->
								xmpp:make_error(IQ, xmpp:err_forbidden(?T("Moderator request is not allowed. The 'with' field is missing"), Lang))
						end;
					_ ->
						NewRSM = limit_max(RSM, NS),
						select_and_send(LServer, Query, NewRSM, FlipPage, IQ, MsgType)
...
store_mam_message(Pkt, U, S, Peer, Nick, Type, Dir) ->
    ...
    Retract = case xmpp:get_subtag(Pkt, #retract_id{}) of
                  #retract_id{id = RID} when RID /= <<"">> ->
                      {true, RID};
                  _ ->
                      false
              end,
...
select_and_send(LServer, Query, RSM, FlipPage, #iq{id = Id, from = From, to = To} = IQ, MsgType) ->
	Flags = case Id of
		        <<"moderator_", _/binary>> ->
			        case proplists:get_value(with, Query) of
						undefined -> all;
...
%%			        case mod_muc_admin:get_room_occupants():
			        {all, true};
		        _ -> all
	        end,
    Ret = case MsgType of
	      chat ->
		  select(LServer, From, From, Query, RSM, MsgType, Flags);
	      _ ->
		  select(LServer, From, To, Query, RSM, MsgType, Flags)
	  end,
    case Ret of
