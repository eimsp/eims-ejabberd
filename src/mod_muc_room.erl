...
normal_state({route, <<"">>,
	...
	true when Type == groupchat ->
		...
		MinMessageInterval =
			case mod_muc_room:get_role(From, StateData) of
				moderator -> 0;
				_ -> trunc(mod_muc_opt:min_message_interval(StateData#state.server_host) * 1000000)
			end,
...
%%		  true;
		      case get_occupant_initial_role(From, get_affiliation(From, StateData), StateData) of
			      moderator -> true;
			      participant -> true;
			      _ -> false
		      end;
...
check_message_for_retractions(Packet,
	...
	#fasten_apply_to{id = ID} = F ->
	    case xmpp:get_subtag(F, #retract_id{}) of
		#retract_id{} ->
...
-spec process_iq_moderate(jid(), iq(), fasten_apply_to(), message_moderate_21(), state()) ->
    {result, undefined, state()} |
    {error, stanza_error()}.
...
process_iq_moderate(From, #iq{type = set, lang = Lang},
		    ...
		    #message_moderate_21{reason = Reason},
		    #state{config = Config, room = Room, host = Host,
		...
		StanzaId ->
		    ...
		    Packet0 = #message{type = groupchat,
					  ...
					  #fasten_apply_to{id = Id, sub_els = [
					      #message_moderated_21{by = By, reason = Reason,
								 retract = #retract_id{id = Id}}
					  ]}]},
