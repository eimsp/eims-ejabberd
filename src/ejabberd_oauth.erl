...
%%process(Handlers, #request{method = 'POST', q = Q} = Request) ->
process(Handlers, #request{q = Q} = Request) ->
    {ok, Re} = re:compile(<<".*(\\n|\\r|<|>|\"|'|\\(|\\)|;|#|&).*">>),
    case lists:flatten([case re:run(V, Re) of nomatch -> []; _ -> false end ||
        {K, V} <- Q, lists:member(K, [<<"redirect_uri">>, <<"state">>, <<"response_type">>, <<"client_id">>, <<"scope">>])]) of
        [] -> process2(Handlers, Request);
        _ ->
            {400, [], ejabberd_web:make_xhtml([?XC(<<"h1">>, <<"Invalid Params (Probably CLRF-injection  is found )">>)])}
    end;
...
process2(_Handlers,
    ...
    case gen_mod:get_module_opt(global, mod_eims_admin, oauth_dialog) of
        true ->
        ...
        _ ->
            {200, [], ejabberd_web:make_xhtml([?XC(<<"h1">>, <<"OAuth dialog form is disabled">>)])}
    end;
process2(_Handlers,
